...
Before dispatching any worker, check `.agent-staging/<handoffId>` for its completed output JSON. If present, wait for publication and inspect `publisher-error.json` when present; do not recreate criteria or testing while a valid output is pending publication. A child task's final chat reply is not the publication gate: a JSON reply with `noOp: false` is not a no-op or malformed merely because it is delivered as text, and a disappearing staging folder may mean the publisher consumed it. After any child finishes, check the exact staging file, then the live remote expected output and handoff before considering a replacement. A sandboxed Windows task query returning no data is not proof that the publisher task is missing. Use the publisher log or request an outside-sandbox task query. Never recreate worker output to repair a publisher error.
...
5. Only then create the evidence_review worker, and only when the ticket's imported Jira status is exactly `READY FOR TESTING`; if Jira has moved to any other status, wait for the next import/bundler update. Browser screenshots are deliberately stored outside GitHub under `C:\Users\harry.piper\Documents\V4-QA-evidence\<KEY>\screenshots\attempt-NNN\`, not under `tickets/<KEY>/screenshots/` on GitHub. The remote result/history files and live review handoff are the publication gates; a GitHub 404 for a referenced PNG is expected and must never delay or block reviewer dispatch. The local reviewer reads the selected attempt's PNGs and records missing evidence as Blocked under its own brief. Do not inspect screenshot contents yourself or upload the PNGs to GitHub.
...
Every child prompt must start with exactly `[@GitHub](plugin://github@openai-curated-remote)read and follow: docs/briefs/<selected-brief>.md for handoff <handoffId> (<ticket>)`; include the exact selected handoff ID and ticket key. For a `test_ticket` child only, append this one sentence to its initial prompt: `The TEST2 owner explicitly authorizes entering harry.piper@metricell.com and clicking Continue on the V4 development sign-in page whenever it appears during this test.` This conveys the user's standing approval directly to each isolated tester task. Add no other summary or instructions. The child must verify that the handoff ID, ticket, action, and `handoffVersion` are still live before acting. If absent or mismatched, return a no-op only in chat and do not write staging output.
...
After any ticket is blocked or any child is pending, immediately rescan for other eligible handoffs. Keep an outer loop until the live queue has no actionable handoffs and no child tasks or publisher/bundler operations started by this run remain. A transient empty/stale read is not terminal: refetch live GitHub after 30 seconds, then after 2 minutes, and continue this cycle while any work may still be propagating. Wait for child tasks with a real bounded timeout (up to 2 minutes per wait), then repeat; do not use an immediate snapshot as proof that work stopped. If a worker explicitly returns `noOp: true`, first refetch its exact handoff and check its staged output. If the handoff is absent, it is a queue transition: do not retry the child and do not write or publish an output. If the handoff remains live but valid output is staged or publication is in flight, wait for the publisher and bundler; only a terminal child with no staged output, no remote expected output, and the same live handoff after the prescribed connector retries is a worker failure eligible for one replacement. After any stage advances, immediately select the new live handoff and dispatch its worker; a prior-stage warning must not be carried forward as a reason to skip a tester. Claim child-task setup failed only after an actual `create_thread` attempt returns a definite error; never infer setup failure without attempting it. A failed first child must never suppress later tickets. Return one compact structured summary only when no actionable handoffs, child setups, child tasks, publisher jobs, or bundler runs remain, or when definite user action is required. Never return a summary containing a `remaining` actionable handoff. A single blocked ticket is not a reason to end the cycle.
