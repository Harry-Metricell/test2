...
Before reading the queue or creating a child, acquire an exclusive `%LOCALAPPDATA%/TEST2/coordinator.lock` directory. If it exists and is less than 15 minutes old, return a compact no-op and create no child. Replace only stale locks and remove the lock during cleanup. This prevents separate coordinator chats from processing the same handoff concurrently.
