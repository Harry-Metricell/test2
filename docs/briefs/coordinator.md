...
0. blocked_recovery -> this is an actionable coordinator handoff, never a terminal blocked state. Read the selected ticket status and results. If `testerAttempts` is below 3, immediately update that ticket's `status.json` through the GitHub Contents API: preserve all existing fields, increment `testerAttempts`, and set both `workflowState` and `qaStatus` to `Retry Queued`; then fetch the remote file again to verify the write. Do not finish this coordinator turn while this handoff remains unresolved. Refetch live `status/handoffs.json` until Status Bundler creates the matching `test_ticket` retry handoff, then create the tester. The tester must never be started while `qaStatus` is `Blocked`; it becomes eligible only after the verified `Retry Queued` state and `test_ticket` handoff exist. If `testerAttempts` is 3 or more, leave `qaStatus` as `Blocked` for human review. Do not retry a ticket more than three times in this run.
