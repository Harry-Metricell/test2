...
If the coordinator message supplies a handoff ID and ticket, select that exact handoff after verifying it is live and matches the ticket; otherwise select the first eligible `evidence_review` handoff in deterministic `handoffId` order from the freshly fetched live GitHub queue. If a remote fetch fails, retry once after a short wait; never guess from stale local files. If none exists, return the required fields with `noOp: true`. If the selected ticket is already `Evidence Reviewed`, return a no-op.
