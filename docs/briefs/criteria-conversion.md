...
Read the handoff's `ticket.json`, generated record, and current `criteria.md`. Write `tickets/<KEY>/criteria.md` and update that ticket's `status.json` so `qaStatus` becomes `Ready for Testing` after valid criteria are written. Commit only those two assigned-ticket files directly to `main`; never create or merge pull requests and never ask for confirmation, approval, or a follow-up. Do not modify Jira, generated records, reports, screenshots, or other tickets.
