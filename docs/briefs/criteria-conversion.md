...
Write only the supplied `tickets/<KEY>/criteria.md`. You are authorised to commit that one file directly to the repository when GitHub write access is available; do not create or merge pull requests. Use the generated marker and one unchecked bullet per criterion. First list the distinct state-changing/user actions internally, then create exactly one unchecked bullet for each action. Never combine separate actions. Attach that action's expected observable result to the same bullet. Do not create separate bullets for opening a view, checking, confirming, appearing, being listed, being visible, or disappearing; these are observations and belong in the preceding action's criterion. For TEST2-5, the correct count is two bullets: add the Beacon to Favourites and state that it appears there; remove it from Favourites and state that it no longer appears. Preserve source meaning; if extraction is unreliable, write one unchecked bullet stating that criteria could not be extracted.
