...
Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Description 'Fast-forward the clean TEST2 Desktop checkout from GitHub; skip safely when local changes exist.' -Force | Out-Null
